...
public class BasicTestEnemy : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		if(Physics.Raycast(transform.position, transform.forward, out hit, 50) ){
			...
			DecreaseCurrentState(fixedCounter, 0.2f, currentState, lookingAtPlayer);
		}

		fixedCounter++;
			switch (agentStateIndex(IncreaseCurrentState(fixedCounter, 0.2f, currentState, lookingAtPlayer)))
			{
		...
		print("current state = " + currentState  + " lookingAtPlayer: " + lookingAtPlayer);
	}
	...
	int IncreaseCurrentState(int Counter, float TimeInSec, int value, bool LookingAtPlayer)
	{
		if (Counter % Mathf.Round(TimeInSec / Time.fixedDeltaTime) == 0 && LookingAtPlayer)
		{
			// Do stuff
			currentState++;
		}
	...
	int DecreaseCurrentState(int Counter, float TimeInSec, int value, bool LookingAtPlayer)
	{
		if (Counter % Mathf.Round(TimeInSec / Time.fixedDeltaTime) == 0 && !LookingAtPlayer)
		{
			// Do stuff
